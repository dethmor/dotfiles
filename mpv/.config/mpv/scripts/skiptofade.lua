...
threshold = 0.9
--detection_span = 0.05
detection_span = 0.232
negation = false
normal_speed = 1
seek_position = 0.0
seekstart_pos = seek_position
...
function stop()
   ...
   local file = io.open(string.format("%s.detect", filepath), "a")
   seekend_pos = mp.get_property_numbre("time-pos")
   io.write(string.format("%s,%f,%f\n", filepath, seekstart_pos, seekend_pos))
   io.flush()
   io.close()
   seekstart_pos = seekend_pos
